...
const checkForReview = data => {
    ...
    if (data.review) {
        ...
        if (lastDate) {
            ...
            if (typeof cycle === 'string' && !reviewDurationLookup.hasOwnProperty(cycle)) {
                throw new Error("Unknown review cycle: " + JSON.stringify(cycle) + " on page: " + data.title);
            }
            const duration = reviewDurationLookup[cycle] || cycle || defaultReviewDuration;
